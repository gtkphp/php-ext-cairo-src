...
php_cairo_glyph_t_setter_int(php_cairo_glyph_t *intern, zval *value, char *name, zval *dest) {
    if (Z_TYPE_P(value)==IS_LONG) {
        ...
        if (strict_types) {
            zend_internal_type_error(1, "Cannot assign %s to property "CAIRO_NS_QUOTE(GTK_NS)"\\cairo_glyph_t::$%s of type float,", type->val, name);
        } else {
            int allow_errors = -1;
            zend_long lval=0;
            double dval=0;
            ...
            zend_error(E_USER_NOTICE, "Cannot assign %s to property "CAIRO_NS_QUOTE(GTK_NS)"\\cairo_glyph_t::$%s of type int,", type->val, name);
        }
    }
}
...
php_cairo_glyph_t_setter_double(php_cairo_glyph_t *intern, zval *value, char *name, zval *dest) {
    if (Z_TYPE_P(value)==IS_DOUBLE) {
        ...
        if (strict_types) {
            ...
            zend_error(E_USER_NOTICE, "Cannot assign %s to property "CAIRO_NS_QUOTE(GTK_NS)"\\cairo_glyph_t::$%s of type float,", type->val, name);
        }
    }
}
...
php_cairo_glyph_t_class_init(zend_class_entry *container_ce, zend_class_entry *parent_ce) {
    php_cairo_glyph_t_get_handlers();
    PHP_CAIRO_INIT_CLASS_ENTRY((*container_ce), "cairo_glyph_t", php_cairo_glyph_t_methods);
