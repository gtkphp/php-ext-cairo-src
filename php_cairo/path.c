...
php_cairo_path_t_setter_int(php_cairo_path_t *intern, zval *value, char *name, zval *dest) {
    if (Z_TYPE_P(value)==IS_LONG) {
        ...
        if (strict_types) {
            zend_type_error("Cannot assign %s to property "CAIRO_NS_QUOTE(GTK_NS)"\\cairo_path_t::$%s of type float,", type_name, name);
        } else {
            int allow_errors = -1;
            zend_long lval=0;
            double dval=0;
            ...
            zend_error(E_USER_NOTICE, "Cannot assign %s to property "CAIRO_NS_QUOTE(GTK_NS)"\\cairo_path_t::$%s of type int,", type_name, name);
        }
    }
}
...
php_cairo_path_t_class_init(zend_class_entry *container_ce, zend_class_entry *parent_ce) {
    php_cairo_path_t_get_handlers();
    PHP_CAIRO_INIT_CLASS_ENTRY((*container_ce), "cairo_path_t", php_cairo_path_t_methods);
